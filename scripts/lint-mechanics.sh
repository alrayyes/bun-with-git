...
# CHANGELOG.md is written by the release job, and .claude/commands and
# .claude/skills are generated by `openspec init`/`openspec update`;
# correcting either is not this script's business.
files=$(git ls-files '*.md' | grep -v '^CHANGELOG.md$' | grep -v '^\.claude/\(commands\|skills\)/')
